	...
	const double fourpiG, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_cdm, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_b, Particles<part_simple,part_simple_info,part_simple_dataType> * pcls_ncdm, double * maxvel, Field<Real> * phi, Field<Real> * chi, Field<Real> * Bi, Field<Real> * source, Field<Real> * Sij, Field<Cplx> * scalarFT, Field<Cplx> * BiFT, Field<Cplx> * SijFT, PlanFFT<Cplx> * plan_phi, PlanFFT<Cplx> * plan_chi, PlanFFT<Cplx> * plan_Bi, PlanFFT<Cplx> * plan_source, PlanFFT<Cplx> * plan_Sij, parameter * params, int & numparam)
{
	...
	if (ic.pkfile[0] != '\0')	// initial displacements & velocities are derived from a single power spectrum
	{
		...
		#ifdef MG
		generateDisplacementField(*scalarFT, sim.gr_flag * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H), pkspline, (unsigned int) ic.seed, ic.flags & ICFLAG_KSPHERE);
		#else
	...
	else					// initial displacements and velocities are set by individual transfer functions
	{
		...
		#ifdef MG
		rescale = 3. * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * (1. + 0.5 * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * ((1. / gsl_spline_eval(mg_cosmo.spline_H,0.98*a,mg_cosmo.acc_H) / gsl_spline_eval(mg_cosmo.spline_H,0.98*a,mg_cosmo.acc_H)) - (8. / gsl_spline_eval(mg_cosmo.spline_H,0.99*a,mg_cosmo.acc_H) / gsl_spline_eval(mg_cosmo.spline_H,0.99*a,mg_cosmo.acc_H)) + (8. / gsl_spline_eval(mg_cosmo.spline_H,1.01*a,mg_cosmo.acc_H) / gsl_spline_eval(mg_cosmo.spline_H,1.01*a,mg_cosmo.acc_H)) - (1. / gsl_spline_eval(mg_cosmo.spline_H,1.02*a,mg_cosmo.acc_H) / gsl_spline_eval(mg_cosmo.spline_H,1.02*a,mg_cosmo.acc_H))) / 0.12);
		#else
		...
		for (i = 0; i < tk_d1->size; i++) // construct phi
			#ifdef MG
			temp1[i] = (1.5 * (gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) - gsl_spline_eval(mg_cosmo.spline_H,1.,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,1.,mg_cosmo.acc_H) * a * a * cosmo.Omega_Lambda) * tk_d1->y[i] + rescale * tk_t1->y[i] / tk_d1->x[i] / tk_d1->x[i]) * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i];
			#else
		...
		if (sim.gr_flag == 0)
		{
			...
			#ifdef MG
			temp2[i] = -3. * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H)  * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i];
			#else
		...
		if (ic.tkfile[0] == '\0')
		{
			if (sim.gr_flag == 0)
			{
				...
				for (i = 0; i < tk_d1->size; i++) // construct gauge correction for N-body gauge velocities
					#ifdef MG
					temp1[i] = -99.5 * gsl_spline_eval(mg_cosmo.spline_H,0.995*a,mg_cosmo.acc_H) * (3. * temp1[i] * M_PI * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] + (temp2[i] + 3. * gsl_spline_eval(mg_cosmo.spline_H,0.99*a,mg_cosmo.acc_H)  * M_PI * tk_t1->y[i] * sqrt(Pk_primordial(tk_d1->x[i] * cosmo.h / sim.boxsize, ic) / tk_d1->x[i]) / tk_d1->x[i] / tk_d1->x[i] / tk_d1->x[i]));
					#else
	...
	else
	{
		...
		if (kFT.coord(0) == 0 && kFT.coord(1) == 0 && kFT.coord(2) == 0)
			...
			#ifdef MG
			solveModifiedPoissonFT(*scalarFT, *scalarFT, fourpiG / a, 3. * sim.gr_flag * (gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) * gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) + fourpiG * cosmo.Omega_m / a));
			#else
	...
	{
		#ifdef MG
		rescale = a / gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) / (1.5 * gsl_spline_eval(mg_cosmo.spline_Omega_m,a,mg_cosmo.acc_Omega_m) + 2. * gsl_spline_eval(mg_cosmo.spline_Omega_rad,a,mg_cosmo.acc_Omega_rad));
		#else
	...
	for (p = 0; p < cosmo.num_ncdm; p++)
	{
		...
		{
			#ifdef MG
			rescale = a / gsl_spline_eval(mg_cosmo.spline_H,a,mg_cosmo.acc_H) / (1.5 * gsl_spline_eval(mg_cosmo.spline_Omega_m,a,mg_cosmo.acc_Omega_m) + gsl_spline_eval(mg_cosmo.spline_Omega_rad,a,mg_cosmo.acc_Omega_rad));
			#else
